...
@section('title', 'Antrean isi saldo')
@section('header', 'Konfirmasi Isi Saldo')
...
<div class="anim-rise flex flex-wrap items-end justify-between gap-3">
    <div class="min-w-0">
        <h3 class="text-xl font-extrabold" style="color: #163C68;">Antrean isi saldo</h3>
        <p class="mt-1 text-sm text-slate-500">Cocokkan dengan mutasi rekening, lalu konfirmasi. Saldo pengguna baru bertambah setelah dikonfirmasi — paling lama menunggu di depan.</p>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm min-w-[720px]">
            <thead><tr class="table-head">
                <th>Diajukan</th><th class="th-c">Pengguna</th><th class="th-c">Nominal</th><th class="th-c">Catatan pengirim</th><th class="th-c">Status</th><th class="th-c">Aksi</th>
            </tr></thead>
            <tbody>
            @forelse ($queue as $p)
                @php
                ...
                @endphp
                <tr class="table-row">
                    <td class="whitespace-nowrap text-slate-500 text-xs">{{ $p->created_at }}</td>
                    <td class="td-c">
                        <p class="font-medium"><span class="marq" title="{{ $p->user?->name }}"><span class="marq-in">{{ $p->user?->name }}</span></span></p>
                        <p class="text-xs text-slate-400"><span class="marq" title="{{ $p->user?->email }}"><span class="marq-in">{{ $p->user?->email }}</span></span></p>
                    </td>
                    <td class="td-c whitespace-nowrap font-extrabold" style="color: #163C68;">Rp{{ number_format($p->amount, 0, ',', '.') }}</td>
                    <td class="td-c text-xs text-slate-500"><span class="marq" title="{{ $p->sender_note }}"><span class="marq-in">{{ $p->sender_note ?? '—' }}</span></span></td>
                    <td class="td-c">@include('super_admin.partials.badge', ['text' => $p->status->value, 'tone' => $tone])</td>
                    <td class="td-c">
                        <a href="{{ route('super_admin.wallet_topups.show', $p->ulid) }}" class="btn btn-navy py-2! px-4! text-xs!">Buka →</a>
                    </td>
                </tr>
            @empty
                <tr><td colspan="6">@include('super_admin.partials.empty', ['title' => 'Antrean kosong', 'hint' => 'Tidak ada isi saldo yang menunggu.'])</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
...
<div id="filterDrawer" class="rdrawer" aria-hidden="true">
    ...
    <aside class="rdrawer-panel sidebar-bg" role="dialog" aria-modal="true" aria-label="Filter isi saldo">
        <div class="flex items-center gap-3 px-6 pt-6">
            ...
            <div class="flex-1 min-w-0">
                <h4 class="font-extrabold text-white leading-tight">Filter isi saldo</h4>
        ...
        <div class="px-6 py-5 space-y-4 text-sm flex-1">
            <div>
                <label class="label-dark" for="f-status">Status isi saldo</label>
                <select id="f-status" name="status" class="field-dark">
                    @foreach (['awaiting_confirmation', 'confirmed', 'rejected', 'cancelled'] as $s)
        ...
        <div class="px-6 pb-6 flex gap-2">
            ...
            <a href="{{ route('super_admin.wallet_topups.index') }}" class="btn btn-ghost flex-1 !bg-white/10 !text-white !border-white/20 hover:!bg-white/20">Reset</a>
        </div>
    </aside>
</div>
</form>
