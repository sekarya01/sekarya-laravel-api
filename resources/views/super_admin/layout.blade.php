...
<head>
    ...
    <style>
        ...
        @keyframes glowPulse { 0%, 100% { box-shadow: 0 0 0 0 rgba(249,115,22,.35); } 50% { box-shadow: 0 0 0 6px rgba(249,115,22,0); } }
        ...
        .ad-4 { animation-delay: .26s; } .ad-5 { animation-delay: .33s; } .ad-6 { animation-delay: .40s; }
        .anim-fade { animation: fadeIn .4s ease both; }

        /* Sidebar */
        .sidebar-bg { background: linear-gradient(178deg, var(--brand-ink) 0%, var(--brand-deep) 45%, var(--brand) 130%); }
        .navlink { display: flex; align-items: center; gap: .7rem; padding: .62rem .8rem; border-radius: .75rem;
            color: #B9C8DC; font-size: .875rem; font-weight: 500; position: relative;
            transition: all .22s ease; border: 1px solid transparent; }
        .navlink:hover { color: #fff; background: rgba(255,255,255,.08); transform: translateX(3px); }
        .navlink svg { width: 1.15rem; height: 1.15rem; flex-shrink: 0; opacity: .85; }
        .navlink-active { color: #fff !important; background: rgba(255,255,255,.12); border-color: rgba(255,255,255,.08); }
        .navlink-active::before { content: ''; position: absolute; left: -.65rem; top: 20%; height: 60%; width: 4px;
        ...
        /* Components */
        .card { background: #fff; border-radius: 1.1rem; border: 1px solid #E4EAF2;
            box-shadow: 0 1px 2px rgba(16,42,76,.05), 0 8px 24px -12px rgba(16,42,76,.12);
            transition: transform .25s ease, box-shadow .25s ease; }
        .card-lift:hover { transform: translateY(-3px); box-shadow: 0 2px 4px rgba(16,42,76,.06), 0 16px 32px -12px rgba(16,42,76,.22); }
        .btn { display: inline-flex; align-items: center; justify-content: center; gap: .5rem; font-weight: 600;
            font-size: .875rem; border-radius: .75rem; padding: .6rem 1.1rem; transition: all .2s ease; cursor: pointer; }
        .btn:active { transform: scale(.97); }
        .btn-accent { background: var(--accent); color: #fff; box-shadow: 0 6px 16px -6px rgba(249,115,22,.6); }
        .btn-accent:hover { background: var(--accent-hover); box-shadow: 0 8px 20px -6px rgba(249,115,22,.7); transform: translateY(-1px); }
        .btn-navy { background: var(--brand); color: #fff; box-shadow: 0 6px 16px -8px rgba(22,60,104,.7); }
        .btn-navy:hover { background: #1D4D86; transform: translateY(-1px); }
        ...
        .field { width: 100%; border-radius: .75rem; border: 1px solid #D7E0EC; background: #FBFCFE;
            padding: .6rem .9rem; font-size: .875rem; transition: all .2s ease; }
        .field:focus { outline: none; border-color: var(--accent); background: #fff;
            box-shadow: 0 0 0 3px rgba(249,115,22,.15); }
        .label { display: block; font-size: .78rem; font-weight: 600; color: #3D546F; margin-bottom: .3rem; }
        .table-head th { font-size: .7rem; text-transform: uppercase; letter-spacing: .08em; color: #7A8DA6;
            font-weight: 700; padding: .8rem 1.1rem; background: #F7FAFD; white-space: nowrap; }
        .table-row { transition: background .15s ease; }
        .table-row:hover { background: #FFF7EF; }
        .table-row td { padding: .9rem 1.1rem; border-top: 1px solid #EDF1F7; vertical-align: middle; }
        ...
        .stat-icon { width: 2.75rem; height: 2.75rem; border-radius: .9rem; display: flex; align-items: center;
            justify-content: center; flex-shrink: 0; }
        .stat-icon svg { width: 1.4rem; height: 1.4rem; }
        .needs-attention { animation: glowPulse 2.4s ease-in-out infinite; }
        #sidebar { transition: transform .3s cubic-bezier(.22,.8,.32,1); }
        @media (max-width: 1023px) {
            #sidebar { position: fixed; inset-block: 0; left: 0; z-index: 40; transform: translateX(-105%); }
...
<div class="flex min-h-screen">
    <div id="sideOverlay" class="fixed inset-0 bg-slate-900/50 z-30 hidden lg:hidden anim-fade"></div>

    <aside id="sidebar" class="sidebar-bg w-[17rem] shrink-0 text-white flex flex-col min-h-screen lg:sticky lg:top-0 lg:h-screen">
        <div class="px-5 pt-6 pb-5">
            <div class="flex items-center gap-3">
                <div class="w-11 h-11 rounded-2xl flex items-center justify-center font-black text-xl text-white shrink-0"
                     style="background: linear-gradient(135deg, var(--accent), #C2570B); box-shadow: 0 8px 20px -6px rgba(249,115,22,.7);">S</div>
                <div>
                    <p class="text-[.68rem] uppercase tracking-[.22em] text-slate-300/80">Sekarya</p>
                    <h1 class="text-lg font-bold leading-tight">Super Admin</h1>
                </div>
            </div>
            @auth('admin_web')
                <div class="mt-4 flex items-center gap-2.5 rounded-xl bg-white/10 border border-white/10 px-3 py-2.5">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-[.68rem] text-slate-300/80 truncate">{{ auth('admin_web')->user()->email }}</p>
                    </div>
                </div>
            @endauth
        </div>
        ...
        @php
            $nav = [
                ...
                ['route' => 'super_admin.verifications.index', 'match' => 'super_admin.verifications.*', 'label' => 'Verifikasi identitas',
                ...
                ['route' => 'super_admin.payments.index', 'match' => 'super_admin.payments.*', 'label' => 'Konfirmasi transfer',
                ...
                ['route' => 'super_admin.users.index', 'match' => 'super_admin.users.*', 'label' => 'Moderasi pengguna',
                ...
                ['route' => 'super_admin.admins.index', 'match' => 'super_admin.admins.*', 'label' => 'Akun pengelola',
                ...
                ['route' => 'super_admin.audit.index', 'match' => 'super_admin.audit.*', 'label' => 'Jejak audit',
        ...
        <div class="p-4">
            ...
            <p class="mt-3 text-center text-[.65rem] text-slate-400/70">Sekarya Console · v1</p>
        </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col min-w-0">
        <header class="sticky top-0 z-20 backdrop-blur bg-white/85 border-b border-[#E4EAF2]">
            <div class="flex items-center gap-3 px-4 sm:px-6 lg:px-8 h-16">
                <button id="menuBtn" class="lg:hidden btn-ghost btn !p-2.5 !rounded-xl" aria-label="Buka menu">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"/></svg>
                </button>
                <div class="min-w-0">
                    <p class="text-[.68rem] uppercase tracking-[.18em] font-bold" style="color: var(--accent);">Konsol Pengelola</p>
                    <h2 class="text-base sm:text-lg font-bold truncate" style="color: var(--brand);">@yield('header', 'Dasbor')</h2>
                </div>
                <div class="ml-auto flex items-center gap-2">
                    <span class="hidden sm:inline-flex items-center gap-1.5 text-xs font-medium text-slate-500 bg-slate-100 rounded-full px-3 py-1.5">
    ...
    @if (session('status'))
        <div class="toast card !border-emerald-200 flex items-start gap-3 p-4" data-toast style="border-left: 4px solid #059669;">
            <div class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center shrink-0">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2.2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
            </div>
...
<script>
    ...
    function closeSide() { sidebar.classList.remove('open'); overlay.classList.add('hidden'); }
