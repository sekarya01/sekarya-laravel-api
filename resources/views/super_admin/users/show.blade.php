...
<div class="mt-4 grid gap-4 lg:grid-cols-2">
    <div class="anim-rise ad-1 card p-5 sm:p-6">
        ...
        <dl class="mt-3 space-y-2.5 text-sm">
            @foreach ([['ULID', $user->ulid, true], ['Telepon', $user->phone ?? '—', false], ['Jenis kelamin', $user->gender?->label() ?? '—', false], ['Tanggal lahir', $user->birth_date?->toDateString() ?? '—', false], ['Umur', $user->age !== null ? $user->age.' tahun' : '—', false], ['Kota', $user->city ?? '—', false], ['Email terverifikasi', (string) ($user->email_verified_at ?? 'belum'), false], ['Terdaftar', (string) $user->created_at, false]] as [$k, $val, $mono])
...
@if ($user->workerProfile)
    <a href="{{ route('super_admin.workers.show', $user->workerProfile) }}" class="anim-rise ad-2 mt-4 card card-lift p-5 flex flex-wrap items-center gap-4">
        <div class="w-11 h-11 rounded-xl flex items-center justify-center text-white font-extrabold shrink-0" style="background: linear-gradient(135deg, #F97316, #C2570B);">
        ...
        <div class="flex-1 min-w-0">
            <p class="font-extrabold" style="color: #163C68;">Punya profil pekerja</p>
            <p class="text-xs text-slate-500">Konteks kerja (reputasi, alamat, keahlian) ada di halaman terpisah.</p>
        </div>
        @if ($user->readyToWork())
            @include('super_admin.partials.badge', ['text' => 'siap kerja', 'tone' => 'orange'])
        @endif
        <span class="text-sm font-bold" style="color: #F97316;">Lihat detail →</span>
    </a>
@endif
